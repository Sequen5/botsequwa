...
SETTINGS
*/
botName = "Sequen Bot" // NAMA BOT
ownerName = "Sequen" // NAMA OWNER
...
const ownerNumber = "6289689542663@s.whatsapp.net" // NOMOR OWNER
	...
	rmln.on('message-new', async (Lan) => {
        ...
        const fakestatus = (teks) => {
			...
			if (isGroup) {
				...
				reply(`Woyy ${sender.split("@")[0]} Jangan Ngomong Kasar Njirr😡`)
				setTimeout(() => {
...
「 *${botName}* 」`
				...
				case 'nuliskiri':
					...
					if (isLimit(sender)) return reply(nad.limitend(pusname, prefix))
					await limitAdd(sender)
					if (!q) return reply(`Teksnya mana kak? Contoh : ${prefix}nuliskiri baik hati`)
				...
				case 'nuliskanan':
					...
					if (isLimit(sender)) return reply(nad.limitend(pusname, prefix))
					await limitAdd(sender)
					if (!q) return reply(`Teksnya mana kak? Contoh : ${prefix}nuliskanan baik hati`)
					reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
...
◯ Biografi : ${abu.biography}`
				...
				case 'tts':
					...
					if (isLimit(sender)) return reply(nad.limitend(pusname, prefix))
					await limitAdd(sender)
					if (args.length < 1) return rmln.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id Halo Sequen`, text, { quoted: Lan })
				...
				case 'bikinquote':
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\ncontoh :\n${prefix}bikinquote aku bukan boneka & Kata Sequen`
...
「 *${botName}* 」`
				...
				case 'avengerstext':
					...
					var ve = gh.split("&")[0];
					var za = gh.split("&")[1];
					if (args.length < 1) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}avengerstext Sequen & Bot`)
					reply(nad.wait())
				...
				case 'gemboktext':
					...
					var ve = gh.split("&")[0];
					var za = gh.split("&")[1];
					if (args.length < 1) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}gemboktext 11 01 2021 & Sequen dan Wina`)
					reply(nad.wait())
				...
				case 'googletext':
					...
					var za = gh.split("&")[1];
					var ga = gh.split("&")[2];
					if (args.length < 1) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}googletext Sequen & Sequen Gans & Sequen Baik`)
					reply(nad.wait())
...
「 *${botName}* 」`
				...
				case 'term':
					...
					exec(cmd, (err, stdout) => {
						if (err) return rmln.sendMessage(from, `root@Sequen:~ ${err}`, text, { quoted: Lan })
						if (stdout) {
